...
当前统一入口另外支持 GTSRB（论文主线）与 CUB-200 单域分类实验，但若后续任务与主线冲突，优先确认是否属于"论文主线实验"还是"单独消融/临时调试"。
...
`scripts/train_refine.py`（`--dataset gtsrb`）当前实现与该口径一致：读取 clean trainset/testset → 加载攻击模型 checkpoint → 实例化 REFINE → 调用 `defense.train_unet(trainset, testset, schedule)`。
...
`scripts/eval_refine.py` 和 `_common.py:manual_refine_eval()` 已按这个口径实现。
...
训练相关：`--epochs`、`--lr`、`--gamma`、`--schedule`、`--disable-schedule`

攻击相关：`--poisoned-rate`、`--y-target`、`--trigger-size`、`--blended-alpha`、`--wanet-grid-k`、`--reflection-dir`、`--reflection-limit`
...
做多投毒率实验时，必须高度重视产物串组问题。**强烈建议显式传入：**
...
原因：`_common.py` 会自动推断最新 checkpoint，如果目录里同时存在多组 poisoned rate，容易误绑到别的实验产物。
...
攻击训练通常会产生：`ckpt_epoch_*.pth`、对应日志目录

REFINE 训练通常会产生：`ckpt_epoch_*.pth`、`label_shuffle.pth`、`log.txt`

REFINE 评估通常会产生：`results.txt`

最终汇总可由 `scripts/collect_final_results.py` 生成，支持 CSV / JSON / Markdown / 论文宽表格式。
...
2. `项目索引文档.md`
3. `实验指令文档.md`
4. `scripts/_common.py`
5. `core/defenses/REFINE.py`
6. 具体攻击脚本与评估脚本
...
- 解释结果时，优先区分：No Defense、REFINE-clean；如果有需要，再单独说明旧版 `REFINE-poisoned` 消融
- 写命令时尽量给出本地版和服务器版两种路径
- 做正式实验建议显式绑定 checkpoint；做临时 smoke test 可依赖自动推断
- 不将 `torchvision://resnet18` URI 列为已验证的正式工作路径；如需预训练加速，推荐使用本地训练好的 benign checkpoint
- 不将 `train_refine.py --pretrain` 误当成分类器提速手段（它对应 UNetLittle 权重，与分类器无关）
